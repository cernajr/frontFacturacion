<template>
    <v-container>
      ...
      <v-dialog v-model="dialogCrear" width="800">
        <v-card>
          <v-card-title> Nueva unidad </v-card-title>
          <v-card-text>
            <v-form ref="form">
              <v-row>
                <v-col cols="12">
      ...
      <v-dialog v-model="isConfirmDialogVisible" persistent width="500">
        <v-card title="Confirmar eliminación">
          <v-card-text>
            ¿Está seguro que desea eliminar esta unidad? Esta acción no se
            puede deshacer.
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        snackbarMessage.value = "Unidad creada exitosamente";
        isSnackbarVisible.value = true;
        closeDialog();
        getData();
      } else {
      ...
      snackbarColor.value = "error";
      snackbarMessage.value = e.data.message || "Error al crear la unidad";
      isSnackbarVisible.value = true;
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await $fetch(
        `${runtimeConfig.public.apiBase}/unidad/${editingItemId.value}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      snackbarColor.value = "success";
      snackbarMessage.value = "Unidad actualizada exitosamente";
      isSnackbarVisible.value = true;
      closeDialog();
      getData();
    } catch (e) {
      snackbarColor.value = "error";
      snackbarMessage.value =
        e.data.message || "Error al actualizar la unidad";
      isSnackbarVisible.value = true;
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await $fetch(`${runtimeConfig.public.apiBase}/unidad/${deleteItemId.value}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json"
        }
      ...
      snackbarColor.value = "success";
      snackbarMessage.value = "Unidad eliminada exitosamente";
      isSnackbarVisible.value = true;
      ...
      snackbarColor.value = "error";
      snackbarMessage.value =
        error.data.message || "Error al eliminar la unidad";
      isSnackbarVisible.value = true;
    } finally {
      closeDialog();
    }
  ...
  const editItem = async (id) => {
    ...
    try {
      const response = await $fetch(
        `${runtimeConfig.public.apiBase}/unidad/${id}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      snackbarColor.value = "error";
      snackbarMessage.value =
        e.data.message || "Error al cargar los datos de la unidad";
      isSnackbarVisible.value = true;
    }
  };
